...
private:
    ...
    public:
        ...
        struct Env {
            Runtime &runtime;
            Scheduler query(beman::execution::get_completion_scheduler_t<
                            beman::execution::set_value_t>) const noexcept {
...
template <typename R1, typename R2> struct set_value_traits<std::pair<R1, R2>> {
    using type = beman::execution::set_value_t(R1, R2);
};
...
private:
    template <typename Receiver> struct ReceiverWrapper {
        ...
        void operator()(int32_t res) noexcept {
            if (res >= 0) {
                beman::execution::set_value(std::move(receiver), res);
        ...
        auto get_stop_token() const noexcept {
            ...
            return beman::execution::get_stop_token(env);
        }
    };
...
private:
    struct Env {
        Runtime &runtime;
        Scheduler query(beman::execution::get_scheduler_t) const noexcept {
            return Scheduler{runtime};
        }
        Scheduler
        query(beman::execution::get_start_scheduler_t) const noexcept {
            return Scheduler{runtime};
        }
        Scheduler
        query(beman::execution::get_delegation_scheduler_t) const noexcept {
            return Scheduler{runtime};
        }
    };
    using ValueType =
        beman::execution::value_types_of_t<Sender, Env, std::tuple,
                                           wait_variant_t>;

public:
    WaitSenderAwaiter(Sender sender, Runtime &runtime)
        : runtime_(runtime), op_state_(beman::execution::connect(
                                 std::move(sender), Receiver{this})) {}
...
public:
    ...
    bool await_suspend(std::coroutine_handle<Promise> handle) noexcept {
        ...
        beman::execution::start(op_state_);
...
private:
    ...
    Runtime &runtime_;
    beman::execution::connect_result_t<Sender, Receiver> op_state_;
