...
private:
    ...
    public:
        ...
        struct Env {
            Runtime &runtime;
            Scheduler query(std::execution::get_completion_scheduler_t<
...
template <typename R> struct set_value_traits {
    using type = std::execution::set_value_t(R);
};
template <typename R1, typename R2> struct set_value_traits<std::pair<R1, R2>> {
    using type = std::execution::set_value_t(R1, R2);
};
...
private:
    template <typename Receiver> struct ReceiverWrapper {
        ...
        void operator()(int32_t res) noexcept {
            if (res >= 0) {
                std::execution::set_value(std::move(receiver), res);
        ...
        auto get_stop_token() const noexcept {
            ...
            return std::execution::get_stop_token(env);
        }
    };
...
private:
    struct Env {
        Runtime &runtime;
        Scheduler query(std::execution::get_scheduler_t) const noexcept {
            return Scheduler{runtime};
        }
        Scheduler query(std::execution::get_start_scheduler_t) const noexcept {
            return Scheduler{runtime};
        }
        Scheduler
        query(std::execution::get_delegation_scheduler_t) const noexcept {
            return Scheduler{runtime};
        }
    };
    using ValueType = std::execution::value_types_of_t<Sender, Env, std::tuple,
                                                       wait_variant_t>;

public:
    WaitSenderAwaiter(Sender sender, Runtime &runtime)
        : runtime_(runtime), op_state_(std::execution::connect(
...
private:
    ...
    Runtime &runtime_;
    std::execution::connect_result_t<Sender, Receiver> op_state_;
